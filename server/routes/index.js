...
router.get('/products', function (req, res, next) {
    res.render('index', { title: 'Our Products', page: "products", displayName: (0, utils_1.UserDisplayName)(req) });
});
router.get('/services', function (req, res, next) {
    res.render('index', { title: 'Our Services', page: "services", displayName: (0, utils_1.UserDisplayName)(req) });
});
...
router.get('/contact-list', utils_1.AuthGuard, function (req, res, next) {
    contact_1.default.find().then(function (data) {
        res.render('index', { title: 'Contact List', page: "contact-list", contacts: data, displayName: (0, utils_1.UserDisplayName)(req) });
    }).catch(function (err) {
        console.error("Error reading contacts from Database" + err);
        res.end();
    });
});
router.get('/edit/:id', utils_1.AuthGuard, function (req, res, next) {
    let id = req.params.id;
    contact_1.default.findById(id).then(function (contactToEdit) {
        res.render('index', { title: 'Edit Contact', page: "edit", contact: contactToEdit, displayName: (0, utils_1.UserDisplayName)(req) });
    }).catch(function (err) {
        console.error(err);
        res.end();
    });
});
router.get('/add', utils_1.AuthGuard, function (req, res, next) {
    console.log("add GET");
    res.render('index', { title: 'Add Contact', page: "edit", contact: '', displayName: (0, utils_1.UserDisplayName)(req) });
});
...
router.post('/edit/:id', utils_1.AuthGuard, function (req, res, next) {
    let id = req.params.id;
...
router.post('/add', utils_1.AuthGuard, function (req, res, next) {
